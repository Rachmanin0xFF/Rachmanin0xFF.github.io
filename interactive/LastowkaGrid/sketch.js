/**
* Adam Lastowka
* IGME-101: Grid (HW 4), 12/7/18
* Made the cellular boximadoohickey, and a whole bunch of 'em, too
*/ 
...
class Cell {
    ...
    constructor(xpos, ypos, w, h) {
		...
		this.slider.parent("sketchcanvas");
    }
...
* Inits width and height text fields
*/
function setup() {
    var cnv = createCanvas(1000, 600); // a little too big for 1920x1080 displays, but not if you go fullscreen in chrome!
    ...
    for(let x = 0; x < 3; x++) {
        testCells[x] = [];
        for(let y = 0; y < 5; y++) {
            testCells[x][y] = new Cell(y*200 + 30, x*200 + 30, 50, 50);
        }
    }
}
...
* clears background, displays cells, does nothing else
*/
function draw() {
    background(255);
    for(let x = 0; x < testCells.length; x++) {
        for(let y = 0; y < testCells[0].length; y++) {
            testCells[x][y].display();
        }
    }
}